...
parser.add_argument('--SS', default=1.0, type=float, help='SCALE_SIGMA')
        ...
        for mt in range(args.num_sample):        
            ...
            for i in range(args.GEN):
                ...
                if i == args.POP-1:
                    ...
                    for k in range(args.L):
                        scale = np.exp(float(np.random.normal(0, args.SS/original_dist_stat[k]['mean'])))
                    ...
                    for k in range(args.L):
                        ...
                        if k in mutate_candidate:
                            scale = np.exp(float(np.random.normal(0, args.SS*step_size/original_dist_stat[k]['mean'])))
                            shift = float(np.random.normal(0, original_dist_stat[k]['std'])- original_dist_stat[k]['mean'])
